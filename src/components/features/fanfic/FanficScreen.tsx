...
import { NavigationProps, GameState, WorldData, AppSettings, SaveFile } from '../../../types';
...
const FanficScreen: React.FC<FanficScreenProps> = ({ onNavigate, onGameStart, initialData }) => {
  ...
  const [showSaveBridgeModal, setShowSaveBridgeModal] = useState(false);
  ...
  const handleOpenSaveBridge = async () => {
    try {
      const allSaves = await dbService.getAllSaves();
      // Sort saves freshest first
      const sorted = (allSaves || []).sort((a, b) => b.updatedAt - a.updatedAt);
      setAvailableSaves(sorted);
      setShowSaveBridgeModal(true);
    } catch (err) {
      console.error("Lỗi nạp saves:", err);
      toast.error("Không thể lấy danh sách savegame từ IndexedDB.");
    }
  };

  const handleLoadSaveFromBridge = (save: SaveFile) => {
    try {
      const worldData = typeof save.data === "string" ? JSON.parse(save.data) : save.data;
      if (!worldData || (!worldData.player && !worldData.world)) {
        toast.error("File lưu này rỗng hoặc không tương thích.");
        return;
      }
      store.importData(worldData);
      toast.success(`Nạp thành công bối cảnh từ File Lưu: "${save.name}"!`);
      setShowSaveBridgeModal(false);
    } catch (err) {
      console.error("Lỗi nạp save:", err);
      toast.error("Lỗi xử lý giải mã dữ liệu savegame.");
    }
  };

  const handleExportSaveFromBridge = async (customSaveName: string) => {
    if (!state.player.name || !state.world.worldName) {
      toast.error("Vui lòng nhập đầy đủ tên nhân vật bối cảnh trước khi xuất!");
      return;
    }
    try {
      const compiledData: WorldData = {
      ...
      const newSave: SaveFile = {
        id: `manual-fanfic-${Date.now()}`,
        name: customSaveName.trim() || `[Đồng Nhân] ${state.world.worldName} (${new Date().toLocaleDateString()})`,
        createdAt: Date.now(),
        updatedAt: Date.now(),
        data: compiledData,
      };

      await dbService.saveGameState(newSave);
      toast.success("Kịch bản Đồng nhân đã được xuất thành Savegame mới trong thư viện!");
      
      const allSaves = await dbService.getAllSaves();
      const sorted = (allSaves || []).sort((a, b) => b.updatedAt - a.updatedAt);
      setAvailableSaves(sorted);
    } catch (err) {
      console.error("Lỗi xuất kịch bản:", err);
      toast.error("Lỗi ghi nhớ save game.");
    }
  };
...
- Thương lái vận tiêu bị phục kích tàn phế ngay cửa ngõ thung lũng đá..."
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {filteredEntities.map(ent => {
              ...
              return (
                <div 
                  key={ent.id} 
                  className="bg-white dark:bg-slate-950/25 border border-slate-200 dark:border-slate-850/80 p-4.5 rounded-2xl hover:border-mystic-accent hover:shadow-[0_4px_12px_-5px_rgba(var(--color-mystic-accent),0.15)] transition-all group relative flex flex-col pt-4 pr-4 pl-4 pb-4"
                >
                  <div className="flex justify-between items-start mb-3">
                    ...
                    <div className="flex gap-1 opacity-100 sm:opacity-0 group-hover:opacity-100 transition-opacity animate-in fade-in">
                      <button 
                  ...
                  <h4 className="font-extrabold text-slate-800 dark:text-slate-100 mb-1.5 truncate text-sm leading-tight text-left">
                    {ent.name}
                  </h4>
                    ...
                    <div className="flex-1 flex flex-col justify-between">
                      <div className="space-y-1 my-1">
                        <div className="flex items-center gap-2 text-[11px] text-slate-500 dark:text-slate-400">
                          ...
                          <span className="font-bold text-slate-600 dark:text-slate-300">Tuổi:</span> {ent.age !== undefined && ent.age !== '' ? `${ent.age}` : 'Chưa rõ'}
                        </div>
                        {ent.narrativeRole && (
                      ...
                      <p className="text-xs text-slate-600 dark:text-slate-400 line-clamp-3 mb-2 pt-1 border-t border-slate-105 dark:border-slate-900/80 leading-relaxed font-normal text-left">
                        {ent.description || ent.personality || ent.background || 'Chưa có thông tin bách khoa.'}
                      </p>
                    </div>
                  ) : (
                    <p className="text-xs text-slate-600 dark:text-slate-400 line-clamp-4 mb-3 flex-1 leading-relaxed font-normal text-left" style={{ wordBreak: 'break-word' }}>
            ...
            {filteredEntities.length === 0 && (
              <div className="col-span-full border-2 border-dashed border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/10 rounded-2xl flex flex-col items-center justify-center p-12 text-slate-400 dark:text-slate-500 text-center w-full">
                <Database size={36} className="mb-3 text-slate-300 dark:text-slate-700 mx-auto" />
  ...
  return (
    <div className="flex flex-col h-full w-full relative overflow-hidden">
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-2 sm:p-4 md:p-8 relative z-10 w-full overflow-hidden mt-safe">
          ...
          <div className="w-full max-w-5xl flex items-center justify-between mb-4 mt-2 gap-2">
            ...
            <div className="flex items-center gap-2 shrink-0 ml-auto md:ml-0">
              ...
              <button 
                onClick={handleOpenSaveBridge} 
                title="Cầu Nối Savegame: Nạp thế giới từ Savegame hoặc Xuất kịch bản trực tiếp thành file lưu" 
                className="text-indigo-600 dark:text-indigo-400 hover:text-indigo-500 transition-all flex items-center gap-1.5 bg-[#e6ebf4] dark:bg-[#0b1329] p-2.5 px-3.5 rounded-2xl shadow-[4px_4px_8px_#cbd2df,-4px_-4px_8px_#ffffff] dark:shadow-[4px_4px_8px_#030610,-4px_-4px_8px_#142042] active:shadow-[inset_2px_2px_4px_#cbd2df,inset_-2px_-2px_4px_#ffffff] dark:active:shadow-[inset_2px_2px_4px_#030610,inset_-2px_-2px_4px_#142042] border border-[#cbd2df]/20 dark:border-[#142042]/15 text-[10px] font-extrabold uppercase tracking-wider cursor-pointer"
              >
                  <SlidersHorizontal size={14} className="text-indigo-500" /> <span>Cầu Nối Save</span>
              </button>
                ...
                <div className="flex justify-end gap-3 pt-2">
                  ...
                  <button
                    onClick={async () => {
                      ...
                      setIsAnalyzing(true);
                      try {
                        // 1. Tự chuyển đổi / tự động chạy phân tích tác phẩm gốc để lấy danh sách nhân vật
                        const result = await fanficAiService.analyzeFanfic(originalContent, originalWorkName, settings || undefined);
                        setAnalyzedCharacters(result.characters);
                        
                        // Cập nhật bối cảnh thế giới từ tóm tắt cuốn sách gốc
                        store.updateWorld('context', result.summary);
                        
                        // Chuyển đổi danh sách nhân vật tác phẩm gốc sang NPC Entities cho bách khoa toàn thư
                        const mappedEntities = (result.characters || []).map((c, index) => {
                          const parentBackground = c.background || '';
                          let fullDesc = `Họ Tên: ${c.name}\nTuổi: ${c.age || 'Chưa rõ'}\nGiới tính: ${c.gender || 'Chưa rõ'}\nTính cách: ${c.personality || 'Chưa rõ'}\nNgoại hình: ${c.appearance || 'Chưa rõ'}\nTiểu sử: ${parentBackground || 'Nhân vật từ tác phẩm gốc'}`;
                          
                          return {
                            id: `orig-char-${Date.now()}-${index}`,
                            name: c.name,
                            type: 'NPC' as const,
                            ...
                            age: c.age || 'Chưa rõ',
                            appearance: c.appearance || 'Chưa rõ',
                            background: parentBackground || 'Chưa rõ',
                            narrativeRole: c.role || 'NPC'
                          };
                        });

                        // 2. Chạy tiếp handleAutoFillAll nhưng truyền bối cảnh này vào
                        let concept = `Đồng nhân của tác phẩm: ${originalWorkName}. Bối cảnh và cốt truyện gốc:\n${result.summary}`;
                        
                        const existingData = {
                          player: state.player,
                          world: { ...state.world, context: result.summary },
                        ...
                        // Gộp cả mappedEntities vào danh sách thực thể bách khoa để chắc chắn có đầy đủ tất cả nhân vật tác phẩm gốc
                        if (data.entities && Array.isArray(data.entities)) {
                          // Loại bỏ các thực thể trùng tên
                          const uniqueEntitiesMap = new Map();
                          mappedEntities.forEach(ent => uniqueEntitiesMap.set(ent.name.toLowerCase().trim(), ent));
                          data.entities.forEach((ent: any) => {
                            uniqueEntitiesMap.set(ent.name.toLowerCase().trim(), ent);
                          });
                          data.entities = Array.from(uniqueEntitiesMap.values());
                        } else {
                          data.entities = mappedEntities;
                        }

                        store.autoFillAll(data);
                      } catch (error: any) {
                        console.error("Sáng tạo đồng nhân thất bại:", error);
                        alert("Lỗi khi khởi tạo nhanh bối cảnh đồng nhân.");
                      } finally {
                        setIsAnalyzing(false);
                        store.setGenerating(false);
      ...
      {/* Savegame Bridge Modal */}
      <AnimatePresence>
        {showSaveBridgeModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 dark:bg-black/75 backdrop-blur-md">
            <motion.div 
              initial={{ opacity: 0, scale: 0.95, y: 15 }}
              animate={{ opacity: 1, scale: 1, y: 0 }}
              exit={{ opacity: 0, scale: 0.95, y: 15 }}
              className="w-full max-w-2xl bg-[#e6ebf4] dark:bg-[#0b1329] border border-[#cbd2df]/35 dark:border-[#142042]/20 rounded-3xl p-6 shadow-2xl flex flex-col max-h-[85vh] overflow-hidden"
            >
              <div className="flex justify-between items-center mb-4 shrink-0">
                <h3 className="text-sm font-black text-indigo-600 dark:text-indigo-400 uppercase tracking-wider flex items-center gap-2">
                  <SlidersHorizontal size={18} className="animate-pulse" /> Cầu Nối Song Đạo: Savegame & Fanfiction Importer
                </h3>
                <button 
                  onClick={() => setShowSaveBridgeModal(false)}
                  className="w-8 h-8 rounded-full flex items-center justify-center bg-[#e6ebf4] dark:bg-[#0b1329] shadow-[3px_3px_6px_#cbd2df,-3px_-3px_6px_#ffffff] dark:shadow-[3px_3px_6px_#030610,-3px_-3px_6px_#142042] active:shadow-[inset_2px_2px_4px_#cbd2df,inset_-2px_-2px_4px_#ffffff] dark:active:shadow-[inset_2px_2px_4px_#030610,inset_-2px_-2px_4px_#142042] text-slate-500 hover:text-red-500 transition-all cursor-pointer border border-[#cbd2df]/20 dark:border-[#142042]/10"
                >
                  <X size={15} />
                </button>
              ...
              <div className="flex-1 overflow-y-auto pr-1 space-y-6 custom-scrollbar text-left py-2">
                {/* Save current setup block */}
                <div className="p-4 bg-white dark:bg-slate-900 rounded-2xl border border-[#cbd2df]/30 dark:border-[#142042]/15 space-y-3">
                  <h4 className="text-xs font-black text-slate-800 dark:text-slate-200 uppercase tracking-widest flex items-center gap-1">
                    🎯 Xuất kịch bản hiện tại thành Savegame mới
                  </h4>
                  <p className="text-[10px] text-slate-500 dark:text-slate-400">
                    Lưu toàn bộ chỉnh sửa nhân vật chính, thực thể, tri thức bách khoa thành file save mới độc lập để chơi ngay lập tức từ menu chính.
                  </p>
                  
                  <div className="flex flex-col sm:flex-row gap-2">
                    <input 
                      type="text"
                      placeholder="Tên file lưu kịch bản (ví dụ: Vũ trụ Đồng nhân Harry..."
                      value={customSaveName}
                      onChange={(e) => setCustomSaveName(e.target.value)}
                      className="flex-1 bg-[#e6ebf4]/40 dark:bg-[#030610]/40 border border-slate-300 dark:border-slate-800 rounded-xl px-3 py-2 text-xs text-slate-800 dark:text-slate-100 placeholder-slate-450 focus:outline-none"
                    />
                    <button
                      onClick={() => {
                        handleExportSaveFromBridge(customSaveName);
                        setCustomSaveName('');
                      }}
                      className="py-2.5 px-4 bg-indigo-500 hover:bg-indigo-600 dark:bg-indigo-600 dark:hover:bg-indigo-750 text-white font-bold text-xs uppercase rounded-xl tracking-wider transition-colors shrink-0 cursor-pointer"
                    >
                      Lưu Mới
                    </button>
                  </div>
                ...
                {/* Import world from saves list */}
                <div className="space-y-3">
                  <h4 className="text-xs font-black text-slate-800 dark:text-slate-200 uppercase tracking-widest">
                    🔌 Nạp bối cảnh Vũ Trụ từ File Lưu có sẵn ({availableSaves.length})
                  </h4>
                  <p className="text-[10px] text-slate-500 dark:text-slate-400">
                    Chọn một file lưu hiện tại của RPG để nạp ngược bối cảnh, nhân vật chính, bách khoa toàn thư của Save đó vào Editor Đồng nhân này để cải tạo!
                  </p>

                  <div className="space-y-2 max-h-[220px] overflow-y-auto custom-scrollbar pr-1">
                    {availableSaves.length === 0 ? (
                      <div className="text-center py-6 text-slate-400 text-xs italic">
                        Chưa có File Lưu Game nào được tạo dưới hệ thống này. Hãy tạo một thế giới hoặc chơi trước!
                      </div>
                    ) : (
                      availableSaves.map((save) => {
                        return (
                          <div 
                            key={save.id}
                            className="flex flex-col sm:flex-row justify-between items-start sm:items-center p-3 bg-white dark:bg-[#080d1d] border border-slate-200 dark:border-slate-800 rounded-2xl hover:border-indigo-500/50 dark:hover:border-indigo-500/50 transition-colors gap-2"
                          >
                            <div className="min-w-0 flex-1 text-left">
                              <span className="font-bold text-xs text-slate-800 dark:text-slate-200 block truncate">{save.name}</span>
                              <span className="text-[9px] text-slate-500 dark:text-slate-500 block font-mono mt-0.5">
                                Cập nhật: {new Date(save.updatedAt).toLocaleString()} | ID: {save.id}
                              </span>
                            </div>
                            <button
                              onClick={() => handleLoadSaveFromBridge(save)}
                              className="py-1.5 px-3 bg-gradient-to-br from-[#e6ebf4] to-[#cbd2df] dark:from-[#0b1329] dark:to-[#142042] text-indigo-600 dark:text-indigo-400 border border-[#cbd2df]/30 rounded-xl text-[10px] font-black uppercase tracking-wider hover:bg-slate-250 dark:hover:bg-slate-800 hover:text-indigo-600 active:scale-95 transition-all self-end sm:self-auto cursor-pointer"
                            >
                              NẠP THẾ GIỚI
                            </button>
                          </div>
              ...
              <div className="pt-4 border-t border-[#cbd2df]/20 dark:border-[#142042]/10 shrink-0 text-right">
                <button
                  onClick={() => setShowSaveBridgeModal(false)}
                  className="py-2.5 px-5 text-xs font-bold bg-[#e6ebf4] dark:bg-[#0b1329] shadow-[3px_3px_6px_#cbd2df,-3px_-3px_6px_#ffffff] dark:shadow-[3px_3px_6px_#030610,-3px_-3px_6px_#142042] active:shadow-[inset_2px_2px_4px_#cbd2df,inset_-2px_-2px_4px_#ffffff] dark:active:shadow-[inset_2px_2px_4px_#030610,inset_-2px_-2px_4px_#142042] text-slate-600 dark:text-slate-400 rounded-xl hover:text-red-500 transition-all cursor-pointer border border-[#cbd2df]/10"
                >
                  Đóng Cầu Nối
                </button>
              </div>
